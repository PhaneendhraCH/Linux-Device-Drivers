...
static struct work_struct *workq; 	// Declare pointer instance of workqueue
...
static int __init my_workqueue_init(void)
{
	if (request_irq(irq, my_interrupt, IRQF_SHARED, "my_interrupt_user", &my_dev_id))
        ...
        workq = kmalloc(sizeof(struct work_struct ),GFP_KERNEL); //Dynamically assigning a workqueue

	INIT_WORK(workq,workq_fn); //Initialise workqueue using macro
	
	printk(KERN_INFO"My workqueue init dynamic function");
	return 0;
}
...
static void __exit my_workqueue_exit(void)
{
	...
	printk(KERN_INFO"My workqueue exit dynamic function");
}
...
MODULE_AUTHOR ("Phani");
MODULE_DESCRIPTION ("Dynamic Work Queue");
MODULE_LICENSE ("GPL");
