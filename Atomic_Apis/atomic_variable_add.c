...
int my_thread1(void *p){
	...
	atomic_add(5,&my_global_var);
	printk(KERN_INFO"my_thread1 invoked and my_global_val after add %d\n",atomic_read(&my_global_var));
	msleep(1000);
	}
	return 0;
}
...
int my_thread2(void *p){
	while(!(kthread_should_stop())){
	atomic_add(5,&my_global_var);
	printk(KERN_INFO"my_thread2 invoked and my_global_val after add %d\n",atomic_read(&my_global_var));
	msleep(1000);
	}
	return 0;
}
