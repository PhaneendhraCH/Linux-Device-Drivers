...
MODULE_AUTHOR("Phani");
MODULE_LICENSE("GPL");
atomic_t my_global_variable = ATOMIC_INIT(0);   // declaring and initialsing the atomic variable.


static struct task_struct *char_thread;	// task_Struct for char_threads
static struct task_struct *char_thread1;	// task_Struct for char_threads

int mydata =0;					// global variable
int my_thread_fun(void *p);			// function prototype
...
int my_thread_fun(void *p)
{
	...
	{		
		...
		printk(KERN_INFO "\n*****************ATOMIC BIT WSIE OPERATORS *************\n");
		
		unsigned long word = 0;
		
		printk(KERN_INFO"Initial Value of word : %x\n",word);
