...
static atomic_t my_global_var  = ATOMIC_INIT(0); // declaring and initialsing to 0
...
int my_thread1(void *p){
	...
	atomic_inc(&my_global_var);
	printk(KERN_INFO"my_thread1 invoked and my_global_val after inc %d\n",atomic_read(&my_global_var));
	msleep(1000);
	}
	return 0;
}
...
int my_thread2(void *p){
	while(!(kthread_should_stop())){
	atomic_inc(&my_global_var);
	printk(KERN_INFO"my_thread2 invoked and my_global_val after inc %d\n",atomic_read(&my_global_var));
	msleep(1000);
	}
	return 0;
}
